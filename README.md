...
- **Настройка параметров эффекта с помощью слайдеров**: Штора управляется по расписанию (ввод времени) или по положению солнца (восход/закат), всегда поддерживая нужное состояние.
- **Ручная тонкая настройка корректировкой кода**: Через кнопку, ИК-пульт, Home Assistant, Яндекс (Алиса), Web страница.
